...
// agent 不再自带凭据 —— 模型凭据由企业管理员在「模型配置」页统一配(ADR-028),
// 按 harness 的 provider 自动注入本企业沙箱。故入参无 api_key/base_url。
...
// 创建智能体(企业管理员)。harness 默认 claude-native。凭据走「模型配置」,不随 agent 下发。
