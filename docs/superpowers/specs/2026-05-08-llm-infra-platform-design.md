...
| Plan 6:BFF 后端 | gateway OIDC 登录/会话/登出(无状态加密 cookie,access TTL≤5min)+ CSRF + `GET /v1/data/jobs`(can()+分页);realm 加固 | **⑤**(GUI 前置) | 🟡 代码就绪(全绿+隔离评审修讫);待 owner 验收+合并(BFF 后端就绪,待 Plan 7 前端)|
...
**出口(当前版)**:① 一行命令清洗→Lance ✅;② Gravitino 元数据可查 ✅(metadata-service,Plan 4 已合并);③ 薄 can() 企业隔离 ✅(S0 交付,管线已接入);服务化 ✅(metadata Plan 4 + data-pipeline Plan 5,契约先行,经 gateway can()+audit);⑤ **真 GUI 经 API 调通** ⏳(BFF Plan 6 后端🟡代码就绪待验收 + 前端 Plan 7,ADR-019;原 SDK/CLI 推迟);DoD 含 code review + CI 绿 + go/no-go 签字(S1 设计 spec §7)。**S1 进度:出口①②③ + 服务化 ✅,⑤ 进展:Plan 6 BFF 后端代码就绪(全绿+隔离评审),待 owner 验收+合并,再做 Plan 7 前端;④ Dev Workspace(Plan 8);owner 直接延长 S1。**
