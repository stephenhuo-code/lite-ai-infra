...
- **P1 补漏(2026-06-29,T3 实测 auth 开启后发现):runner tunnel 在多用户 auth 下 403。** P1 跑的是 `AUTH_ENABLED=0`,未覆盖 auth-on 的 runner 回连。**根因(源码级)**:omnigent `host_tunnel`(`server/routes/host_tunnel.py:139-151`)用 managed launch token(`X-Omnigent-Host-Token`→`resolve_launch_token`→owner)认证、**绕过身份头**;而兄弟端点 `runner_tunnel`(`server/routes/runner_tunnel.py:343-348`)**无此 token 旁路**——auth 开启即强求用户身份头。自托管 docker managed 沙箱里的 runner 只带 binding token、无用户凭据、内网无代理 → `get_user_id` 返 None → 4004。**与 header/oidc 无关**(oidc 模式下 runner 同样无 cookie,一样 403);**P1 因 auth 关而 `auth_provider is None` 走单用户回退才跑通**。omnigent 内置 managed provider 是云 SDK(Modal/Daytona,SDK 注入凭据进 runner),**无"普通-docker managed under 多用户 auth"**——正是我们要建的新东西,fork 侧补齐符合预期。
  - **决策(owner 拍板 2026-06-29)= fork 修 `runner_tunnel`**:给 runner tunnel 加 managed-token 旁路,**镜像 omnigent 自己 host_tunnel 已有的模式**(binding-token→runner→session→owner,服务端解出 owner,绕身份头)。最小、循官方模式、可 upstream,**非手搓/私改**(是补齐两个兄弟端点的一致性)。提交进我们 fork(模型 C)→ 重编译 server 镜像 → bump submodule。否决备选:① sidecar 代理为 runner WS 注身份(代理无法把 runner 解析成 owner,不成立)② 退单用户(破 FR-004/005)。
