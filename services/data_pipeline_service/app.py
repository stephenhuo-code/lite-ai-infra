...
def build_app(runner, audit: AuditWriter, uploader=None, metadata=None):
    ...
    def prepare(body: PrepareJobRequest, request: Request,
        ...
        spec = JobSpec(job_id=job_id, dataset=body.dataset, enterprise_id=ent,
                       role=ctx.role_in(EnterpriseId(ent)) or "member",   # owner 模型:企业级角色快照(无组)
    ...
    def list_jobs(status: str | None = None,
        ...
        ent = enterprise_of(ctx)
        visible = []
        for j in runner.list_jobs():                       # 纯取数(含 enterprise_id/owner_user 投影)
    ...
    def request_upload(body: RawUploadRequest, ctx: Context = Depends(context_from_request)):
        ...
        if not d.allow:                                       # deny → 零副作用 + 审计
            ...
            return JSONResponse(status_code=403, content={"reason": d.reason})
        try:
            grant = uploader.create_grant(name=body.dataset, enterprise_id=ent, user_id=ctx.user,
        ...
        _audit(audit, ctx, ent, f"raw/{body.dataset}", "data.upload", "allow", "",
    ...
    def complete_upload(raw_id: str, body: CompleteUploadRequest | None = None,
        ...
        try:
            ...
            _audit(audit, ctx, ent, f"raw/{rec['name']}", "data.upload", "deny", "object-missing")
            return JSONResponse(status_code=409, content={"reason": str(e)})
        _audit(audit, ctx, ent, f"raw/{rec['name']}", "data.upload", "allow", "complete",
