...
def test_submit_returns_202_job(tmp_path):
    ...
    r = c.post("/v1/data/prepare", headers=_hdr("u-a", ["e-0001"]),
...
def test_platform_admin_submit_denied_no_side_effect(tmp_path):
    ...
    r = c.post("/v1/data/prepare", headers=_hdr("u-pa", ["e-0001"], ["platform-admin"]),
               json={"dataset": "cc3m", "source_dataset": "cc3m-raw"})
...
def test_get_owner_ok_non_owner_403_unknown_404(tmp_path):
    ...
    c, store, _ = _client(tmp_path)
    jid = c.post("/v1/data/prepare", headers=_hdr("u-a", ["e-0001"]),
                 json={"dataset": "cc3m", "source_dataset": "cc3m-raw"}).json()["id"]
    assert c.get(f"/v1/data/jobs/{jid}", headers=_hdr("u-a", ["e-0001"])).status_code == 200
    assert c.get(f"/v1/data/jobs/{jid}", headers=_hdr("u-b", ["e-0001"])).status_code == 403
    assert c.get("/v1/data/jobs/nope", headers=_hdr("u-a", ["e-0001"])).status_code == 404
...
def test_prepare_stores_source_dataset_and_job_projects_it(tmp_path):
    ...
    c, store, _ = _client(tmp_path)
    body = c.post("/v1/data/prepare", headers=_hdr("u-a", ["e-0001"]),
