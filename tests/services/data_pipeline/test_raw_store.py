...
def _spec(rid="raw-1", owner="u-a", ent="e-0001"):
    # owner 模型(ADR-024):RawSpec 按 owner_user(上传用户),路径段按 user
    return RawSpec(raw_id=rid, name="cc3m", owner_user=owner, enterprise_id=ent,
                   sub=owner, oss_key=f"{ent}/{owner}/raw/cc3m/part-0.tar", upload_id=None)
...
def test_create_then_read_projects_pending(tmp_path):
    ...
    assert r["owner_user"] == "u-a" and r["oss_key"].endswith("raw/cc3m/part-0.tar")
...
def test_list_raw_returns_projection_with_isolation_fields(tmp_path):
    ...
    s.create(_spec("raw-1", "u-a")); s.create(_spec("raw-2", "u-b"))
    rows = s.list_raw()
    assert {r["id"] for r in rows} == {"raw-1", "raw-2"}
    assert all("enterprise_id" in r and "owner_user" in r for r in rows)   # handler can() 过滤依赖

def test_load_spec_roundtrips_upload_id(tmp_path):
    s = RawDatasetStore(str(tmp_path))
    s.create(RawSpec("raw-m", "cc3m", "u-a", "e-0001", "u-a", "e-0001/u-a/raw/cc3m/big.tar", "UP-123"))
