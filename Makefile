...
run-data-pipeline: ; LITEAI_JWKS_URL=$(JWKS) JOBS_DIR=./.dev/jobs OSS_ENDPOINT=http://localhost:9000 OSS_ACCESS_KEY=minio OSS_SECRET_KEY=minio123 OSS_REGION=us-east-1 DATA_BUCKET=lite-ai AUDIT_BUCKET=lite-ai uv run uvicorn services.data_pipeline_service.main:app --port 8003 --reload
